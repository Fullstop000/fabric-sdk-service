...
public class CoinTransactionServiceImpl implements CoinTransactionService {
    ...
    public String queryTransactionInfo(String queryStr,Channel channel) throws YunphantCoinException {
        ...
        failureResponseProcess(failureResponses,"queryTransactionRecord");
        ...
        channel.sendTransaction(successfulResponse).thenApply(transactionEvent -> {
            try {
                if (transactionEvent.isValid()){
                    ...
                    String error = "Fail to commit transaction : invalid transaction event";
                    logger.error(error);
                    throw new YunphantCoinException("Fail to commit transaction : invalid transaction event");
                }
     ...
     * @throws YunphantCoinException
     */
    @Override
    public void createAccount(String jsonStr, Channel channel) throws YunphantCoinException {
        // TODO: 2017/9/19 refactor this to reduce the duplication
        try {
            client = YunphantHFClient.getInstance().getClient();
        } catch (Exception e) {
            String error = "Error creating client instance";
        ...
        failureResponseProcess(failureResponse,"createAccount");
     ...
     */
    @Override
    public String queryBalance(String userID, Channel channel) throws YunphantCoinException {
        try {
            client = YunphantHFClient.getInstance().getClient();
        } catch (Exception e) {
            String error = "Error creating client instance";
        ...
        request.setArgs(new String[]{userID});
        request.setProposalWaitTime(PROPOSAL_WAIT_TIME);
        Map<String, byte[]> map = new HashMap<>();
        map.put("HyperLedgerFabric", "TransactionProposalRequest:JavaSDK".getBytes(UTF_8)); //Just some extra junk in transient map
        map.put("method", "TransactionProposalRequest".getBytes(UTF_8)); // ditto
        ...
        failureResponseProcess(failureResponses,"queryBalance");
        return successfulResponses.get(0).getProposalResponse().getResponse().getPayload().toStringUtf8();
    }
